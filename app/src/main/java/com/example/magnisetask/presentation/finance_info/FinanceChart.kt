...
fun FinanceChart(
    ...
    val upperValue = remember(infos) {
        infos.maxOfOrNull { it.c } ?: 0.0
    }

    val lowerValue = remember(infos) {
        infos.minOfOrNull { it.c } ?: 0.0
    }
    ...
    Canvas(modifier = modifier) {
        ...
        (0..4).forEach { i ->
            drawContext.canvas.nativeCanvas.apply {
                drawText(
                    String.format("%.4f", (lowerValue + priceStep * i)),
                    30f,
        ...
        val strokePath = Path().apply {
            ...
            for (i in infos.indices) {
                ...
                val leftRatio = (infos[i].c - lowerValue) / (upperValue - lowerValue)
                val rightRatio = (nextInfo.c - lowerValue) / (upperValue - lowerValue)
